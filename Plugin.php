...
class FreewindMusic_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        echo '<link rel="stylesheet" href="' . Helper::options()->siteUrl . __TYPECHO_PLUGIN_DIR__ . '/FreewindMusic/aplayer/APlayer.css">';

        $color = Freewind_Helper::theme_color();
        $file = __FREEWIND_ROOT__ . '/static/css/color/' . $color;
        $fp = fopen($file, 'r');
        $content = fread($fp, 1024);
        preg_match('/@top-color: (.*)/', $content, $matche_top);
        ?>
        <style>
            .player .aplayer-body {
    ...
    {
        echo '<script src="' . Helper::options()->siteUrl . __TYPECHO_PLUGIN_DIR__ . '/FreewindMusic/aplayer/Meting.js"></script>';
        echo '<script src="' . Helper::options()->siteUrl . __TYPECHO_PLUGIN_DIR__ . '/FreewindMusic/aplayer/APlayer.js"></script>';
        ?>
        <script>
            $(function () {
                $(".top-list.fr.hide-sm").prepend(`<li style="margin: 0;">
<meting-js
